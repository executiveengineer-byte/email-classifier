...
from flask import (
    Flask, request, render_template, send_from_directory, send_file, jsonify, abort, redirect, url_for, flash, jsonify, Response, flash, make_response, session
)
...
# =========================================================
# === START: CORRECTED USER AND APP CONFIGURATION BLOCK ===
# =========================================================
...
# This is now the ONE AND ONLY User class definition
class User(UserMixin, dict):
    ...
    def name(self):
        """
        Safely returns the user's name from the dictionary.
        Falls back to the user's email if the 'name' key is not found.
        """
        return self.get('name', self.get('email'))

# =======================================================
# === END: CORRECTED USER AND APP CONFIGURATION BLOCK ===
# =======================================================
...
def classify_email(email_text, subject_text=""):
    ...
    # ===================================================================
    # === START: NEW OVERRIDE LOGIC TO REDUCE CONFUSION             ===
    # ===================================================================
    # This rule specifically targets the case where a security/admin email
    # is mistakenly flagged as a complaint.
    strong_other_keywords = ["security alert", "your account", "invoice overdue", "password", "subscription", "verify your email", "login attempt"]
    
    # If the final prediction is 'complaint' but the ML model wasn't highly confident,
    # AND we find a strong "other" keyword, we override the prediction.
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user_doc and check_password_hash(user_doc.get('password', ''), password):
            ...
            if next_page:
                # Flash the message before redirecting to the 'next' page as well
                flash(f'Welcome, {user_obj.name}!', 'success')
                return redirect(next_page)

            # 2. Flash the personalized welcome message ONCE for all roles.
...
# In testapp.py, add this new route after the sales_dashboard_personal route
...
# In testapp.py, add these new functions
...
# In testapp.py
# In testapp.py, add this new function with your other data-prep functions
...
def prepare_executive_report_data():
    ...
    # ============================ THE FIX: REAL TEAM DATA CALCULATION ============================
    # This logic is copied from get_dashboard_data to ensure consistency.
    team_performance = []
    ...
    context = {
        ...
        'team_data': team_performance, # Use the real data
        'top_performers': sorted(team_performance, key=lambda x: x.get('score', 0), reverse=True), # Use the real data
    }

    # Generate charts and add them to the context, now using REAL data
...
# In testapp.py, add this new function

@app.errorhandler(403)
def forbidden_error(error):
    """
    Renders a custom, user-friendly 403 Forbidden page.
    """
...
@role_required('exec')
def manage_leads():
    """Shows all leads and allows executives to assign them."""
    all_leads = list(leads_collection.find().sort("created_at", -1))
    
    # Get a list of all users with the 'sales' role to populate the dropdown
    sales_people = list(db.users.find({'role': 'sales'}, {'name': 1, '_id': 0}))
    
    return render_template('leads.html', leads=all_leads, sales_people=sales_people)
    ...
    if request.method == 'POST':
        try:
            ...
            if not lead_name or not lead_email:
                flash("Lead Name and Email are required.", "danger")
            ...
            # THE FIX: Ensure we are using the robust .name property from our User class
            ...
            new_lead = {
                ...
                "status": "new",
                "assigned_to": assigned_to_name, # Use the corrected name
                "created_at": datetime.now()
            }
...
@app.route('/export/my_leads/<export_type>')
@login_required
def export_my_leads(export_type):
    ...
    try:
        ...
        # Remove MongoDB's internal _id and select/reorder columns
        if '_id' in df.columns:
            df = df.drop(columns=['_id'])
        
        # Define the columns you want in your export and their order
        ...
        if export_type == 'csv':
            # Use an in-memory string buffer to avoid saving a file on the server
            ...
            # Use an in-memory bytes buffer for Excel files
            ...
            # For PDF, we render an HTML template first
            html = render_template('export_leads_pdf.html', leads=df.to_dict('records'), user_name=sales_person_name)
            
            # Use pdfkit to convert the HTML to a PDF
            WKHTMLTOPDF_PATH = r'C:\Program Files\wkhtmltopdf\bin\wkhtmltopdf.exe' # Ensure this path is correct
            PDFKIT_CONFIG = pdfkit.configuration(wkhtmltopdf=WKHTMLTOPDF_PATH)
            pdf_file = pdfkit.from_string(html, False, configuration=PDFKIT_CONFIG)
...
@nocache
def logout():
    # THE FIX: Clear any leftover flash messages from the previous page.
    session.pop('_flashes', None)

    # Now, proceed with the standard logout process.
    logout_user()
    
    # Flash the one, clean message you want the user to see.
