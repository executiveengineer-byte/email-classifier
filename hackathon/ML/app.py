...
class User(UserMixin, dict):
    ...
    def name(self):
        """
        Safely returns the user's name.
        Falls back to the user's email if the name is not set.
        """
        # .get('name', ...) will try to find the 'name' key.
        # If it's not found, it will use the value after the comma as a default.
        # Here, we cleverly use another .get() for the default, making it very robust.
...
def create_sla_trend_chart(df):
    """
    Creates a smoothed SLA compliance area-line chart like the Email Volume Trend style.
    """
    try:
        if not df:
            return ""
        ...
        ax.set_xticklabels(df['date'].dt.strftime('%Y-%m-%d'), rotation=30, color="#555")

        ax.set_ylim(0, 110)
        ax.set_yticks([0, 20, 40, 60, 80, 100])
        ax.tick_params(axis='y', colors='#aaa')

        ax.set_title("SLA Compliance Trend", fontsize=13, color="#333", pad=15)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ...
        print("Error generating SLA chart:", e)
        return ""
